...
void Colony::addAnt2(sf::Vector2f spawn_loc)
{
	...
	Ant& newant = AntContainer[AntContainer.size() - 1];
	//newant.initAnt(0.1f, spawn_loc, ant_skin, pblocker_systm_ptr, food_systm_ptr, m_pheromatrix_ptr);
	newant.initAnt2(0.1f, spawn_loc, ant_skin);
	float init_faceangle{ static_cast<float>(rand() / (RAND_MAX / 360)) };

	// add environment system pointers 
	newant.m_pheroSystem_ptr = this->m_pheroMatrix_ptr;
...
//NEW VERSION
void Colony::initColony2(PathBlockSystem* pb_system, sf::Texture* skin, PheroMatrix* phero_system,
	SpatialPartition* partition_system, FoodSystem* food_system, Terrain* terrain_system)
{
	...
	std::cout << "Colony Initialized with pblock add:" << m_PBlockerSystm_ptr << "\n";
	std::cout << "size of Ant = " << AntContainer.size() << "\n";
	this->m_totalResource = 0;

	m_chole_position = sf::Vector2f(1000.0f, 1000.0f);
	Chole.setFillColor(sf::Color::Black);
	Chole.setRadius(15.0f);
	Chole.setOrigin(Chole.getRadius(), Chole.getRadius());
	Chole.setPosition(m_chole_position);
